...
def test_transverse_profiles_3d():
    ...
    print("std_y = ", std_y)
    assert xp.abs(std_x - std_th_x) / std_th_x < 0.01
    assert xp.abs(std_y - std_th_y) / std_th_y < 0.01

    # TransverseProfileFromData
    print("TransverseProfileFromData")
    lo = (-40.0e-6, -40.0e-6)
    hi = (40.0e-6, 40.0e-6)
    x = xp.linspace(lo[0], hi[0], 200)
    y = xp.linspace(lo[1], hi[1], 100)
    dx = x[1] - x[0]
    w0 = 10.0e-6
    x0 = 10.0e-6
    X, Y = xp.meshgrid(x, y, indexing="ij")
    intensity_data = xp.exp(-((X - x0) ** 2 + Y**2) / w0**2)
...
def test_longitudinal_profiles():
    ...
    t = xp.linspace(t_peak - 4 * tau_fwhm, t_peak + 4 * tau_fwhm, npoints)
    omega = xp.linspace(omega_0 - 4 * omega_fwhm, omega_0 + 4 * omega_fwhm, npoints)
    wavelength_axis = 2.0 * xp.pi * c / omega  # Note: monotonically decreasing
    ...
    t_peak_gaussian = t[xp.argmax(xp.abs(field_gaussian))]
    print("t_peak_th = ", t_peak)
    print("t_peak = ", t_peak_gaussian)
    assert xp.abs(t_peak_gaussian - t_peak) / t_peak < 0.01

    ff_gaussian = field_gaussian * xp.exp(-1.0j * omega_0 * t)
    cep_phase_gaussian = xp.angle(ff_gaussian[xp.argmax(xp.abs(field_gaussian))])
    print("cep_phase_th = ", cep_phase)
    print("cep_phase = ", cep_phase_gaussian)
    assert xp.abs(cep_phase_gaussian - cep_phase) / cep_phase < 0.02
    ...
    t_peak_super_gaussian = t[xp.argmax(xp.abs(field_super_gaussian))]
    print("t_peak_th = ", t_peak)
    print("t_peak = ", t_peak_super_gaussian)
    assert xp.abs(t_peak_super_gaussian - t_peak) / t_peak < 0.01

    ff_super_gaussian = field_super_gaussian * xp.exp(-1.0j * omega_0 * t)
    cep_phase_super_gaussian = xp.angle(
        ff_super_gaussian[xp.argmax(xp.abs(field_super_gaussian))]
    )
    print("cep_phase_th = ", cep_phase)
    print("cep_phase = ", cep_phase_super_gaussian)
    assert xp.abs(cep_phase_super_gaussian - cep_phase) / cep_phase < 0.02
    ...
    t_peak_cos = t[xp.argmax(xp.abs(field_cos))]
    print("t_peak_th = ", t_peak)
    print("t_peak = ", t_peak_cos)
    assert xp.abs(t_peak_cos - t_peak) / t_peak < 0.01

    ff_cos = field_cos * xp.exp(-1.0j * omega_0 * t)
    cep_phase_cos = xp.angle(ff_cos[xp.argmax(xp.abs(field_cos))])
    print("cep_phase_th = ", cep_phase)
    print("cep_phase = ", cep_phase_cos)
    assert xp.abs(cep_phase_cos - cep_phase) / cep_phase < 0.02
    ...
    t_peak_gaussian_data = t[xp.argmax(xp.abs(field_data))]
    print("t_peak_th = ", t_peak)
...
def test_from_array_profile():
    ...
    t = xp.linspace(lo[2], hi[2], npoints[2])
    X, Y, T = xp.meshgrid(x, y, t, indexing="ij")
    e0 = 4.0e12  # Approx a0 = 1 for wavelength = 800 nm
    wx = 3.0e-6
    wy = 5.0e-6
    tau = 5.0e-15
    E = 1j * e0 * xp.exp(-(X**2) / wx**2 - Y**2 / wy**2 - t**2 / tau**2)
    wavelength = 0.8e-6
    pol = (1, 0)
    axes = {"x": x, "y": y, "t": t}
    dim = "xyt"
...
def test_flattened_gaussian_profile():
    ...
    radlineout_nf_cp = (
        xp.abs(las_nf_cp.grid.get_temporal_field()[0, :, int(npoints[1] / 2)]) ** 2
    )
    radlineout_ff = (
        xp.abs(las_ff.grid.get_temporal_field()[0, :, int(npoints[1] / 2)]) ** 2
    )

    err = xp.sum(
        xp.abs(xp.abs(radlineout_nf_cp) ** 2 - xp.abs(radlineout_ff) ** 2)
    ) / xp.sum(xp.abs(radlineout_ff) ** 2)

    assert err < 1e-2

    energy_ff = compute_laser_energy(dim, las_ff.grid)
    energy_nf_cp = compute_laser_energy(dim, las_nf_cp.grid)
    assert xp.abs(energy_ff - energy_nf_cp) / energy_ff < 1e-4

    w0_ff = get_w0(las_ff.grid, dim)
    w0_nf_cp = get_w0(las_nf_cp.grid, dim)
    assert xp.abs(w0_nf_cp - w0_ff) / w0_ff < 1e-2
