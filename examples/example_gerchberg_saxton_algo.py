...
laser.grid.set_temporal_field(
    xp.abs(laser.grid.get_temporal_field()) * xp.exp(1j * phase3D)
)
...
im0 = ax[0, 0].imshow(xp.abs(field[:, :, tIndx]) ** 2, extent=extent, cmap="PuRd")
...
im1 = ax[0, 1].imshow(
    xp.angle(field[:, :, tIndx]) * phaseMask, extent=extent, cmap="coolwarm"
)
...
laser_calc.grid.set_temporal_field(
    xp.abs(laser_calc.grid.get_temporal_field()) * xp.exp(1j * phaseBackward)
)
...
phaseResidual = xp.angle(field_calc[:, :, tIndx]) - xp.angle(field[:, :, tIndx])
phaseResidual -= phaseResidual[int(npoints[1] / 2), int(npoints[0] / 2)]
maxPhaseRes = xp.max(xp.abs(phaseResidual) * phaseMask)
im3 = ax[1, 1].imshow(
...
im4 = ax[0, 2].imshow(xp.abs(field_bw[:, :, tIndx]) ** 2, extent=extent, cmap="PuRd")
...
im5 = ax[0, 3].imshow(xp.angle(field_bw[:, :, tIndx]), extent=extent, cmap="coolwarm")
...
phaseResidual = xp.angle(field_bw[:, :, tIndx]) - phaseBackward[:, :, tIndx]
phaseResidual -= phaseResidual[int(npoints[1] / 2), int(npoints[0] / 2)]
maxPhaseRes = xp.max(xp.abs(phaseResidual) * phaseMask)
im6 = ax[0, 4].imshow(
    phaseResidual * phaseMask,
...
im7 = ax[1, 2].imshow(xp.abs(field_fw[:, :, tIndx]) ** 2, extent=extent, cmap="PuRd")
...
im8 = ax[1, 3].imshow(xp.angle(field_fw[:, :, tIndx]), extent=extent, cmap="coolwarm")
...
phaseResidual = xp.angle(field_fw[:, :, tIndx]) - phaseForward[:, :, tIndx]
phaseResidual -= phaseResidual[int(npoints[1] / 2), int(npoints[0] / 2)]
maxPhaseRes = xp.max(xp.abs(phaseResidual) * phaseMask)
im9 = ax[1, 4].imshow(
    phaseResidual * phaseMask,
